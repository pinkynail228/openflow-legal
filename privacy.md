...
Mermaid authoring, Preview, Apply, инструкция и legend не требуют backend и не
отправляют source в сеть.

## Какие данные обрабатываются

Только при добровольном использовании API OpenFlow передаёт через
...
- Выбранный язык хранится отдельной несекретной записью `clientStorage`.
- После явного применения ограниченная API-история, применённый `FlowSpec`, origin
  и техническая metadata без secrets сохраняются в private plugin data созданной
  OpenFlow group. Mermaid apply сохраняет уже существующую bounded API-историю.
...
Пользователь может удалить key на вкладке API, удалить OpenFlow group из документа
